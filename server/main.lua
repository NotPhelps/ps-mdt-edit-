...
RegisterServerEvent("qb-mdt:getid")
AddEventHandler("qb-mdt:getid", function(cid,time)
        local src = source
...
RegisterServerEvent("qb-mdt:getid2")
AddEventHandler("qb-mdt:getid2", function(cid,fine)
    local src = source
    local biller = QBCore.Functions.GetPlayer(src)
    local Ply = QBCore.Functions.GetPlayerByCitizenId(cid)
    local billed = QBCore.Functions.GetPlayerByCitizenId(cid)
    local amount2 = round(fine * 0.10)
    local amount3 = round(fine * 0.90)
     print(fine)
    if biller.PlayerData.job.name == "police" then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                    Ply.Functions.RemoveMoney('bank', fine, "paid-fine")
                    print(fine)
                    ...
                    TriggerEvent('qb-bossmenu:server:addAccountMoney', 'police', amount3)
                     biller.Functions.AddMoney('bank', amount2, "Fine Funds")
            else
                TriggerClientEvent('QBCore:Notify', src, 'You Cannot Fine Yourself', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'Person Not Online', 'error')
        end
    else
